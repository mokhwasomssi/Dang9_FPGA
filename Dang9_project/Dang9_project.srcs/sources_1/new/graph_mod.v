...
// bar의 위치 결정
/*---------------------------------------------------------*/
assign bar_y_t = bar_y_reg; //bar의 top
...
always @ (posedge clk or posedge rst) begin
    if (rst | game_stop) bar_y_reg <= (MAX_Y-BAR_Y_SIZE)/2; //game이 멈추면 중간에서 시작
    else if (refr_tick) 
...
always @ (posedge clk or posedge rst) begin
    if(rst | game_stop) begin
        ...
        else if (reach_wall) ball_vx_reg <= BALL_V; //벽에 부딪히면 오른쪽으로 
        else if (reach_bar) ball_vx_reg <= -1*BALL_V; //바에 튕기면 왼쪽으로
    end  
end
...
assign miss = (miss_ball==1 && refr_tick==1)? 1 : 0; //bar가 ball을 놓침, miss를 1클럭 pulse로 만들기 위해 refr_tick과 AND 시킴
